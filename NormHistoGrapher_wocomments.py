    ...
    class MercatorLatitudeTransform(mtransforms.Transform):
        ...
        def transform_non_affine(self, a):
            ...
            if masked.mask.any():
                ...
                # return  np.array([np.log(np.log(1/(1- (1-np.exp(-1*((xi/p2)**p0))) ))) for xi in a])
                # return np.array([np.log(np.log(1/(1- xi))) for xi in masked])
                # #  np.array([np.log(1-xi) for xi in masked])
                return np.array([np.log(xi) for xi in masked])
    ...
    def on_visibility(self, event):
        ...
        if notebookFrame.index(self) != 0:
            ...
            if ('tabs' not in fig_dict and canContainer not in notebookFrame.tabs()) or notebookFrame.index(self) != notebookFrame.index(canContainer):
                # # notebookFrame.index(self), notebookFrame.index(canContainer)
                # # notebookFrame.nametowidget(canContainer) in notebookFrame.tabs()
...
# apply(lambda x: fxy(x['A'], x['B']), axis=1)
    ...
    elif purpose == 'saveAndQuit':
        titleLabel = ttk.Label(frame, text = "Would you like to save your work before quitting?", font = SMALL_FONT)
        titleLabel.grid(sticky = "nw", row=0, column= 1, pady = 8, padx = 8)

        B1 =  ttk.Button(popup, text="Cancel")
        B1 = ttk.Button(popup, text="Cancel", command = lambda: app_close())
        B1.grid(row=15, column=3,  sticky="ew", pady=5, padx=5)

        B2 =  ttk.Button(popup, text="Okay")
        B2 = ttk.Button(popup, text="Okay", command = lambda: save_and_close())
...
def saveWorkspace(context = None):
    ...
    # 'dictionary length', len(fig_dict)
...
def megaSaveWorkspace():
    ...
    if fig not in fig_dict and len(fig_dict) == 0:
...
def excel_plottable_prob_plot(specs, xPoints, yPoints, plotType):
    ...
    xPointsNew = [xPoint for i, xPoint in enumerate(xPoints[:]) if str(yPoints[i]) not in ['inf', '-inf', 'nan'] and str(xPoint) not in ['inf', '-inf', 'nan']]
    yPoints = [yPoints[i] for i, xPoint in enumerate(xPoints[:]) if str(yPoints[i]) not in ['inf', '-inf', 'nan'] and str(xPoint) not in ['inf', '-inf', 'nan']]
    xPoints = xPointsNew
