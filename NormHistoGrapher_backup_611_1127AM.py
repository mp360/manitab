...
class TabFrame(tk.Frame):
    ...
    def on_visibility(self, event):
        ...
        if notebookFrame.index(self) != 0:
            ...
            if canContainer not in notebookFrame.tabs() or notebookFrame.index(self) != notebookFrame.index(canContainer):
...
def popupmsg(purpose):
    ...
    def process_anova_input(purpose):
        ...
        if purpose == 'ANOVA1':
            ...
            try:
                ...
                tkMessageBox.showerror("Invalid Alpha", "Please check that you have entered a non-zero float value for Alpha.\n")
                return
    ...
    if purpose == 'Table':
        ...
        importButton.grid(row=0, column=2, columnspan = 2, sticky="w", pady=5, padx=15)
        ...
        addButton =  ttk.Button(treeFrame, text=">>", command = lambda: moveToListBox(listbox1, listbox2))
        # importButton = ttk.Button(frame, text="Okay", command = prime_nxt_popup(purpose))
        addButton.grid(row=5, column=2,  sticky="w", pady=5, padx=5)
        removeButton =  ttk.Button(treeFrame, text="<<", command = lambda: moveToListBox(listbox2, listbox1))
        # importButton = ttk.Button(frame, text="Okay", command = prime_nxt_popup(purpose))
