...
class StartPage(tk.Frame):
    def __init__(self, parent, controller):
        ...
        figure.subplots_adjust(bottom = 0.25, right = .85, top = .85)
        figure.subplots_adjust(wspace=.3, hspace=.35)
        figure.patch.set_facecolor('#E0E0E0')
        figure.patch.set_alpha(0.7)
        axes2 = figure.add_subplot(111)
        maximizedAxis = axes2

        # output = file('sinus2.pickle','rb')
        # dictPickle = pl.load(output)
        # output.close()
        # keys = dictPickle.keys()
        
        # output = file('savedCanvas.pickle','rb')
        # dictPickle = pl.load(output)
        # output.close()
        # keys = dictPickle.keys()
        ...
        # canvas = FancyFigureCanvas(keys[0], canContainer)
        ...
        # pl.dump(dictPickle,file('sinus2.pickle','wb'))
        pl.dump(dictPickle,file('savedCanvas.pickle','wb'))
