...
class TabFrame(tk.Frame):
    ...
    def on_visibility(self, event):
        ...
        if notebookFrame.index(self) != 0:
            ...
            if notebookFrame.index(self) != notebookFrame.index(canContainer):
            ...
            if self.tab_selected_before_this:
...
def renameTab(label):
    ...
    if label in current_tabs_dict:
        ...
        current_tabs_dict[label] += [str(datetime.datetime.now())]
...
def reopenGraph(tabLabel, tabID, context = None):
    ...
    fig = None
    for figkey in loaded_fig_dict:
        if loaded_fig_dict[figkey]['lastSaved'] == lastSavedTimeExpected:
            fig_dict[figkey] = dict(loaded_fig_dict[figkey])
            fig = figkey
            break
    if fig: print("fig was obtained :)")
    loaded_fig_dict = None    
    # print fig_dict[fig]

    #TODO INVESTIGATE
    if canContainer != None:
        print("if this graph came from a menu, delete the entry in menu!!")
        oldCanContainer = canContainer
        canContainer = notebookFrame.nametowidget(tabID)
        go_to_next_slide(fig)
...
def addTab(name = 'Blank Chart*', selectedFromMenu = False):
    ...
    notebookFrame.add(canContainer, text=name, compound=tk.BOTTOM)
...
class SeaofBTCapp(tk.Tk):
    def __init__(self, *args, **kwargs):
        ...
        tk.Tk.wm_title(self, "Histogram with PDF Fit")
        ...
        anova_menu.add_command(label = "One-Way AnOVa")
        anova_menu.add_command(label = "Two-Way AnOVa")
        ...
        wkspmenu.add_separator()
        wkspmenu.add_command(label = "View Workspace Directory", command = lambda: showThisWorkspace())
...
def megaSaveWorkspace():
    ...
    if fig in fig_dict and fig_dict[fig]['sessionType'] in ['RELA', 'PDF']:
        ...
        for axes in fig_dict[fig]['dictOfAxes'].keys():
            if 'axesScales' in fig_dict[fig]['dictOfAxes'][axes]:
    ...
    with open('savedWkspc1.pickle', 'wb') as handle:
        pl.dump(loaded_fig_dict, handle, protocol=pl.HIGHEST_PROTOCOL)
        handle.close()
...
def deleteTab(tabIndex):
    ...
    if tabIndex < len(notebookFrame.tabs()):
        ...
        elif len(notebookFrame.tabs()) > 1 and fig != None:

            print "got here getting new canContainer"
            for i in range(0, len(notebookFrame.tabs())):
                thisID = notebookFrame.tabs()[i]
                if 'normal' == notebookFrame.tab(thisID, 'state') and i != 0:
                    canContainer = notebookFrame.nametowidget(thisID)
                    break
            if canContainer:
                notebookFrame.nametowidget(hiddenGraphsTab).set_tab_selected_before_this(notebookFrame.index(canContainer))
                print("open this graph is TRUE!!!")
                tabLabel = notebookFrame.tab(canContainer, "text")
        ...
        # elif len(notebookFrame.tabs()) > 1 and not canContainer:
        #     print "got here getting new canContainer"
        #     for i in range(0, len(notebookFrame.tabs())):
        #         thisID = notebookFrame.tabs()[i]
        #         if 'normal' == notebookFrame.tab(thisID, 'state') and i != 0:
        ...
        # else: # no tabs remaining
        #     print "KILLL the graph!!"
    ...
    if canContainer != None and str(canContainer) == str(thisID):
        ...
        if fig in fig_dict and fig_dict[fig]['loaded']:
            ...
            with open('savedWkspc1.pickle', 'wb') as handle:
                pl.dump(loaded_fig_dict, handle, protocol=pl.HIGHEST_PROTOCOL)
                handle.close()
        elif fig in fig_dict:
            del fig_dict[fig]

        loaded_fig_dict = None
        canContainer = None
        thisLabel = notebookFrame.tab(thisID, "text")
    if thisLabel != None:
        print("CASE 11")
        ...
        if thisLabel in current_tabs_dict:
            ...
            if not thisID:
                ...
                for figkey in loaded_fig_dict.keys():
                    if loaded_fig_dict[figkey]['lastSaved'] == timestamp:
                        del loaded_fig_dict[figkey]
                with open('savedWkspc1.pickle', 'wb') as handle:
                    pl.dump(loaded_fig_dict, handle, protocol=pl.HIGHEST_PROTOCOL)
                    handle.close()
            ...
            if (dataType.get() == "Date (mm/dd/yyyy)"):
                ...
                try:
                    ...
                    tkMessageBox.showerror("Zero Variation Error", "Data at header " + str(gotten)
                        + " in file " + str(dataFile) + " has no variation. Failed to draw curve.\n\n" + " Error:" + str(e))
